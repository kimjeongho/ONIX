...
//    List<BoardCommentData> items = new ArrayList<BoardCommentData>();
   ...
   List<BoardCommentData> items = new ArrayList<BoardCommentData>();    // 댓글 작성
    public void put(BoardData data){    //게시글 목록에서
        this.data = data;
...
//        DetailArtistData data = items.get(position);
        ...
        } else if(position > 0 && position < items.size() + 1 ){    // 네일 타입 페이지
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if(position == 0){
            ...
            ((BoardCommentHolder)holder).setBoardComment(items.get(position - 1));
            return;
        }
